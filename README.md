...
This pass (2026-09-07 ~11:21 AM MT): `python scripts/localnet_recreate.py` created Watchdog **appId 1108** at confirmed round **84** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1109**, set_keeper, set_max_age(1000), interested-party `report(42)`, and inner-called `watch` (1 inner). Global after listen: last_value=42, stale=0, watch_count=1. LocalNet last-round after listen: 90. Did not spend the TestNet bank. Did not poke upkeep 81 or 87. Prior LocalNet appIds 1025/1026 are superseded.
