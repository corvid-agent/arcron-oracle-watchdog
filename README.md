...
This pass (2026-09-09 ~10:45 AM MT): `python scripts/localnet_recreate.py` created Watchdog **appId 1053** at confirmed round **42** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1054**, set_keeper, set_max_age(1000), interested-party `report(42)`, and inner-called `watch` (1 inner). Global after listen: last_value=42, stale=0, watch_count=1, last_watch_round=48. LocalNet last-round after listen: 48. Did not spend the TestNet bank. Did not poke upkeep 81 or 87. Prior LocalNet appIds 1014/1015 (and 1147/1148, 1108/1109) are superseded.
