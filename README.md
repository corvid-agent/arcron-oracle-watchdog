...
This pass (2026-09-02 ~12:42 PM MT): `python scripts/localnet_recreate.py` created Watchdog **appId 1170** at confirmed round **135** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1171**, set_keeper, set_max_age(1000), interested-party `report(42)`, and inner-called `watch` (1 inner). Global after listen: last_value=42, stale=0, watch_count=1, last_watch_round=141. LocalNet last-round after listen: 141. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
