...
This pass (2026-09-09 ~1:57 PM MT): `python scripts/localnet_recreate.py` created Watchdog **appId 1081** at confirmed round **65** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1082**, set_keeper, set_max_age(1000), interested-party `report(42)`, and inner-called `watch` (1 inner). Global after listen: last_value=42, stale=0, watch_count=1, last_watch_round=71. LocalNet last-round after listen: 71. Did not spend the TestNet bank. Did not poke upkeep 81 or 87. Prior LocalNet appIds 1053/1054 (and 1014/1015, 1147/1148, 1108/1109) are superseded.
