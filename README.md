...
This pass (2026-09-08 ~2:14 PM MT): `python scripts/localnet_recreate.py` created Watchdog **appId 1014** at confirmed round **11** (`createTxid` in `docs/localnet.json`). Then `python scripts/localnet_listen.py` created mock keeper **1015**, set_keeper, set_max_age(1000), interested-party `report(42)`, and inner-called `watch` (1 inner). Global after listen: last_value=42, stale=0, watch_count=1, last_watch_round=17. LocalNet last-round after listen: 17. Did not spend the TestNet bank. Did not poke upkeep 81 or 87. Prior LocalNet appIds 1147/1148 (and 1108/1109) are superseded.
