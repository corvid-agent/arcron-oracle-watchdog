...
def test_deploy_json_stays_testnet_undeployed() -> None:
    ...
    assert DEPLOY.get("reportTxid") in ("", None)
...
def test_listen_json_is_localnet_not_testnet() -> None:
    ...
    assert int(g.get("last_value") or 0) == 42
    assert int(g.get("watch_count") or 0) >= 1
    assert int(g.get("stale") or 0) == 0
    calls = listen.get("calls") or []
    methods = [c.get("method") for c in calls]
    assert "set_keeper" in methods
    assert "set_max_age" in methods
    assert "report" in methods
    assert "watch" in methods
