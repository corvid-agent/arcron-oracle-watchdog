#!/usr/bin/env python3
"""LocalNet listen: set_keeper + set_max_age + report + mock inner-call watch.
...
SET_MAX_AGE = Method.from_signature("set_max_age(uint64)void")
REPORT = Method.from_signature("report(uint64)uint64")
MK_WATCH = Method.from_signature("watch(uint64)void")
MAX_AGE = 1000
REPORT_VALUE = 42
...
def main() -> None:
    ...
    print(f"target Watchdog appId={app_id} creator={creator}", file=sys.stderr)

    addr, pk = funded_account(algod, kmd, prefer=creator)
    if addr == BANK:
        sys.exit("refusing to sign as TestNet bank")
    if creator and addr != creator:
        sys.exit(f"refusing: signer {addr} is not Watchdog creator {creator}")
    ...
    wd_addr = get_application_address(app_id)
    calls: list[dict] = []
    ...
    rec = call_abi(
        algod, addr, signer, app_id, SET_MAX_AGE, method_args=[MAX_AGE]
    )
    rec["maxAge"] = MAX_AGE
    calls.append(rec)
    print(f"set_max_age txid={rec['txid']} round={rec['round']}", file=sys.stderr)

    rec = call_abi(
        algod, addr, signer, app_id, REPORT, method_args=[REPORT_VALUE]
    )
    rec["value"] = REPORT_VALUE
    rec["hook"] = "report"
    rec["via"] = "interested_party_pull"
    calls.append(rec)
    print(f"report txid={rec['txid']} round={rec['round']}", file=sys.stderr)
    ...
    rec["hook"] = "watch"
    rec["via"] = "mock_keeper.watch"
    calls.append(rec)
    print(
        f"watch via mock txid={rec['txid']} round={rec['round']} inners={rec['innerCount']}",
        file=sys.stderr,
    )
    ...
    payload = {
        ...
        "calls": calls,
        "global": {
            "last_value": after.get("last_value", 0),
            "last_report_round": after.get("last_report_round", 0),
            "stale": after.get("stale", 0),
            "watch_count": after.get("watch_count", 0),
            "last_watch_round": after.get("last_watch_round", 0),
            "keeper_app": after.get("keeper_app", 0),
            "max_age": after.get("max_age", 0),
        },
        "notes": (
        ...
        json.dumps(
            {
                ...
                "appId": app_id,
                "mockKeeperAppId": mock_id,
                "last_value": payload["global"]["last_value"],
                "stale": payload["global"]["stale"],
                "watch_count": payload["global"]["watch_count"],
